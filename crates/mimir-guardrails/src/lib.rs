...
impl Guardrails {
    ...
    pub async fn classify_memory(&self, memory: &Memory) -> Result<ClassificationResult> {
        ...
        let security_level = if detected_pii.is_empty() {
            SecurityLevel::Safe
        } else if detected_pii.iter().any(|pii| {
            matches!(
                pii.pii_type,
...
mod tests {
    ...
    fn test_detect_ssn() {
        ...
        for (content, should_detect) in test_cases {
            let detections = guardrails.detect_pii(content);
            let has_ssn = detections
                .iter()
                .any(|d| matches!(d.pii_type, PiiType::SocialSecurityNumber));
    ...
    fn test_detect_credit_card() {
        ...
        for (content, should_detect) in test_cases {
            let detections = guardrails.detect_pii(content);
            let has_cc = detections
                .iter()
                .any(|d| matches!(d.pii_type, PiiType::CreditCard));
    ...
    fn test_detect_ip_address() {
        ...
        for (content, should_detect) in test_cases {
            let detections = guardrails.detect_pii(content);
            let has_ip = detections
                .iter()
                .any(|d| matches!(d.pii_type, PiiType::IpAddress));
    ...
    async fn test_classify_sensitive_content() {
        ...
        assert!(matches!(
            classification.detected_pii[0].pii_type,
            PiiType::Email
        ));
    }
    ...
    fn test_pii_position_accuracy() {
        ...
        assert_eq!(
            &content[detection.start_pos..detection.end_pos],
            "user@domain.com"
        );
    }
    ...
    fn test_pii_type_equality() {
        ...
        assert_eq!(
            PiiType::Other("custom".to_string()),
            PiiType::Other("custom".to_string())
        );
        assert_ne!(
            PiiType::Other("a".to_string()),
            PiiType::Other("b".to_string())
        );
    }
