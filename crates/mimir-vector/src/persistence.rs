...
impl VectorStorePersistence {
    ...
    pub async fn save_store(
        ...
        fs::create_dir_all(&self.vault_path).map_err(|e| {
            VectorError::Persistence(format!("Failed to create vault directory: {}", e))
        })?;
    ...
    fn save_metadata(&self, metadata: &VectorStoreMetadata) -> VectorResult<()> {
        let data = bincode::serialize(metadata).map_err(|e| {
            VectorError::Serialization(format!("Failed to serialize metadata: {}", e))
        })?;

        let mut file = fs::File::create(&self.metadata_path).map_err(|e| VectorError::Io(e))?;

        file.write_all(&data).map_err(|e| VectorError::Io(e))?;
    ...
    fn load_metadata(&self) -> VectorResult<VectorStoreMetadata> {
        let mut file = fs::File::open(&self.metadata_path).map_err(|e| VectorError::Io(e))?;

        let mut data = Vec::new();
        file.read_to_end(&mut data)
            .map_err(|e| VectorError::Io(e))?;

        let metadata: VectorStoreMetadata = bincode::deserialize(&data).map_err(|e| {
            VectorError::Serialization(format!("Failed to deserialize metadata: {}", e))
        })?;
    ...
    async fn save_index_and_vectors(
        ...
        let data = bincode::serialize(&vector_data).map_err(|e| {
            VectorError::Serialization(format!("Failed to serialize vector data: {}", e))
        })?;

        let mut file = fs::File::create(&self.vectors_path).map_err(|e| VectorError::Io(e))?;

        file.write_all(&data).map_err(|e| VectorError::Io(e))?;

        // Save index structure (placeholder for now)
        let index_data = b"placeholder_index_data";
        let mut index_file = fs::File::create(&self.index_path).map_err(|e| VectorError::Io(e))?;
    ...
    async fn load_index_and_vectors(
        ...
        // Load vector data
        let mut file = fs::File::open(&self.vectors_path).map_err(|e| VectorError::Io(e))?;

        let mut data = Vec::new();
        file.read_to_end(&mut data)
            .map_err(|e| VectorError::Io(e))?;

        let vector_data: VectorDataForPersistence = bincode::deserialize(&data).map_err(|e| {
            VectorError::Serialization(format!("Failed to deserialize vector data: {}", e))
        })?;
    ...
    pub fn delete_store(&self) -> VectorResult<()> {
        if self.metadata_path.exists() {
            fs::remove_file(&self.metadata_path).map_err(|e| VectorError::Io(e))?;
        }

        if self.index_path.exists() {
            fs::remove_file(&self.index_path).map_err(|e| VectorError::Io(e))?;
        }

        if self.vectors_path.exists() {
            fs::remove_file(&self.vectors_path).map_err(|e| VectorError::Io(e))?;
        }
