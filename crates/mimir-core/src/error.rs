...
mod tests {
    ...
    fn test_error_display_messages() {
        ...
        assert_eq!(vector_error.to_string(), "Vector store error: Index not found");
        
        let encryption_error = MimirError::Encryption("Key derivation failed".to_string());
        assert_eq!(encryption_error.to_string(), "Encryption error: Key derivation failed");
        
        let guardrails_error = MimirError::Guardrails("PII detected".to_string());
        assert_eq!(guardrails_error.to_string(), "Guardrails error: PII detected");
        
        let compression_error = MimirError::Compression("Model load failed".to_string());
        assert_eq!(compression_error.to_string(), "Compression error: Model load failed");
        ...
        assert_eq!(config_error.to_string(), "Configuration error: Invalid port");
