...
use mimir_core::{crypto::CryptoManager, Config, Result};
...
pub async fn initialize_vault(config: &Config) -> Result<()> {
    ...
    if !app_dir.exists() {
        ...
        std::fs::create_dir_all(app_dir).map_err(|e| {
            mimir_core::MimirError::Initialization(format!("Failed to create app directory: {}", e))
        })?;
    }

    // Initialize crypto manager (creates keyset if needed)
    info!("Initializing cryptographic system...");
    let _crypto_manager = CryptoManager::new(&keyset_path).map_err(|e| {
        mimir_core::MimirError::Initialization(format!("Failed to initialize crypto: {}", e))
    })?;
    ...
    if !config.get_database_path().exists() {
        ...
        std::fs::write(&config.get_database_path(), "").map_err(|e| {
            mimir_core::MimirError::Initialization(format!("Failed to create database file: {}", e))
        })?;
    }
