...
impl Embedder {
        ...
        for expected in &expected_inputs {
            if !session_inputs.iter().any(|input| input.name == *expected) {
                return Err(VectorError::OnnxModel(format!(
                    ...
                    expected,
                    session_inputs.iter().map(|i| &i.name).collect::<Vec<_>>()
                )));
            }
        }
    ...
    fn get_embedding_dimension(session: &mut Session) -> VectorResult<usize> {
        ...
        let dummy_token_type_ids = Tensor::from_array(([1, 3], vec![0i64; 3])).map_err(|e| {
            VectorError::OnnxModel(format!("Failed to create dummy token_type_ids: {}", e))
        })?;
        let dummy_attention_mask = Tensor::from_array(([1, 3], vec![1i64; 3])).map_err(|e| {
            VectorError::OnnxModel(format!("Failed to create dummy attention_mask: {}", e))
        })?;
        ...
        let embedding_tensor = outputs[0].try_extract_array::<f32>().map_err(|e| {
            VectorError::OnnxModel(format!("Failed to extract dummy embedding: {}", e))
        })?;
    ...
    pub async fn embed(&mut self, text: &str) -> VectorResult<Vec<f32>> {
        ...
        let input_ids_tensor = Tensor::from_array(([1, input_len], tokens)).map_err(|e| {
            VectorError::EmbeddingGeneration(format!("Failed to create input tensor: {}", e))
        })?;
        let token_type_ids_tensor =
            Tensor::from_array(([1, input_len], token_type_ids)).map_err(|e| {
                VectorError::EmbeddingGeneration(format!(
                    "Failed to create token_type_ids tensor: {}",
                    e
                ))
            })?;
        let attention_mask_tensor =
            Tensor::from_array(([1, input_len], attention_mask)).map_err(|e| {
                VectorError::EmbeddingGeneration(format!(
        ...
        let embedding_tensor = outputs[0].try_extract_array::<f32>().map_err(|e| {
            VectorError::EmbeddingGeneration(format!("Failed to extract embedding: {}", e))
        })?;

        // Extract [CLS] token embedding (first token)
        let embedding_data = embedding_tensor.as_slice().ok_or_else(|| {
            VectorError::EmbeddingGeneration("Failed to get embedding data".to_string())
        })?;
    ...
    fn normalize_embedding(embedding: &[f32]) -> VectorResult<Vec<f32>> {
        ...
        if magnitude < f32::EPSILON {
            return Err(VectorError::EmbeddingGeneration(
            ...
            VectorError::OnnxModel(msg) => {
                assert!(
                    msg.contains("Failed to load model")
                        || msg.contains("Tokenizer file not found")
                );
            }
            _ => panic!("Expected OnnxModel error"),
        }
