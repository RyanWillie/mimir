...
use crate::batch_ops::{BatchConfig, BatchOperations, SearchQuery, VectorInsert};
use crate::error::{VectorError, VectorResult};
use crate::hnsw_store::{SearchResult, SecureVectorStore};
use crate::memory_manager::{MemoryConfig, MemoryManager};
...
impl ThreadSafeVectorStore {
    ...
    ) -> VectorResult<crate::batch_ops::BatchInsertResult> {
    ...
    ) -> VectorResult<crate::batch_ops::BatchSearchResult> {
        if let Some(batch_ops) = &self.batch_ops {
...
mod tests {
    ...
    async fn test_memory_limits() {
        ...
        let store =
            ThreadSafeVectorStore::new(temp_dir.path(), 128, Some(memory_config), None).unwrap();
    ...
    async fn test_persistence() {
        ...
        // Load from disk
        let loaded_store = ThreadSafeVectorStore::load(temp_dir.path(), None, None, None)
            .await
            .unwrap();
