...
impl Compressor {
    ...
    pub async fn decompress_memory(&self, compressed_data: &[u8]) -> Result<String> {
        ...
        let content = String::from_utf8(compressed_data.to_vec())
            .map_err(|e| mimir_core::MimirError::Compression(format!("UTF-8 decode error: {}", e)))?;
...
mod tests {
    ...
    async fn test_compress_large_content() {
        ...
        let memory = MemoryBuilder::new().with_content(large_content.clone()).build();
    ...
    async fn test_compress_decompress_roundtrip() {
        ...
        // Decompress
        let decompressed_content = compressor.decompress_memory(&compressed_data).await.unwrap();
    ...
    fn test_calculate_similarity_identical() {
        let compressor = create_test_compressor();
        let memory1 = MemoryBuilder::new().with_content("Identical content").build();
        let memory2 = MemoryBuilder::new().with_content("Identical content").build();
    ...
    fn test_calculate_similarity_different() {
        let compressor = create_test_compressor();
        let memory1 = MemoryBuilder::new().with_content("Completely different").build();
        let memory2 = MemoryBuilder::new().with_content("Totally unrelated").build();
    ...
    fn test_calculate_similarity_partial() {
        ...
        let memory2 = MemoryBuilder::new().with_content("This is different").build();
    ...
    async fn test_compression_with_special_characters() {
        ...
        // Test round-trip
        let decompressed = compressor.decompress_memory(&compressed_data).await.unwrap();
